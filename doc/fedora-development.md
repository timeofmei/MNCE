...
- CMake 3.30.5 或更高版本
- Ninja
- Qt 6.11.2，包含 Core、Concurrent、Network、Sql、Widgets 和 Multimedia
- Wayland 或 X11 图形会话
- 可用的桌面音频输出；Fedora 默认的 PipeWire 环境即可

项目在 CMake 中要求 Qt 6.11.2 精确版本。不能通过放宽版本约束来适配开发机上的其他 Qt 版本。
...
当 Fedora 软件源仍提供 Qt 6.11.2 时，可以使用系统包。Qt 包必须在安装命令中显式限定为 `6.11.2`，不能依赖软件源当时的默认候选版本：

```bash
sudo dnf install \
    'qt6-qtbase-devel-6.11.2-*' \
    'qt6-qtmultimedia-devel-6.11.2-*' \
    'qt6-qtwayland-6.11.2-*'
```
...
上述版本限定只约束本次安装事务，不会阻止 Fedora 后续更新系统 Qt。不要使用 `dnf versionlock` 冻结全部 `qt6-*` 包：Fedora 桌面和其他系统应用也会使用这些包，长期锁定可能阻碍正常的兼容性及安全更新。如果系统 Qt 在后续更新中不再是 6.11.2，应改用独立 Qt 安装方案。

#### 方案二：使用独立 Qt 安装

如果当前 Fedora 软件源不提供 Qt 6.11.2，或者需要不受系统 Qt 更新影响的长期稳定开发环境，应使用 Qt 官方安装器安装 Linux `gcc_64` 构建以及 Qt Multimedia 模块。安装方法见 Qt 官方的 [Get and Install Qt](https://doc.qt.io/qt-6/get-and-install-qt.html) 和 [Qt Online Installer](https://doc.qt.io/qt-6/qt-online-installation.html)。
...
`qtpaths6 --qt-version` 必须输出 `6.11.2`。CMake 必须不低于 `3.30.5`。
...
先确认 `qtpaths6 --qt-version`，再检查 `CMAKE_PREFIX_PATH` 是否指向 Qt 6.11.2。修正环境后使用新的构建目录重新配置，不修改项目的 Qt 精确版本要求。
