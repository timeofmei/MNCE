...
项目自有代码和允许静态组合的宽松许可证依赖可以静态链接；Qt 和 FFmpeg 在正式发行中使用动态链接。优先采用 Qt 官方构建、Qt Multimedia 默认 FFmpeg 后端和 Qt 部署工具提供的运行时布局，不在没有实际兼容性问题时自行替换媒体后端。所有依赖仍需固定版本并提供可复现构建。所有平台的 Qt 版本锁定为 6.11.2；Windows 使用动态版 `msvc2022_64` kit 作为开发基线。
